...
def plot_and_save(figure_size, legend_size, save_path=r"Dashboard\dashboard_1.png"):
    ...
    # 添加文本標籤
    # plt.text(x, y, s, ha, va): 用於在 Figure 中的某位置增加文本標籤。
    # x, y: 文本 x,y 軸位置
    # s: 顯示文本本人
    # ha, va: 水平對齊, 垂直對齊  
    ...
    plt.title("訪問次數與不重複訪問者", fontsize=16)
    plt.xticks(x, df["日期"], fontsize=12)
    plt.yticks(fontsize=12)

    # plt.gca(): Get Current Axes 獲取當前子圖對象
    # spines 是圖表中的邊框線
...
def plot_dual_axis_chart(figure_size, legend_size, save_path=r"Dashboard\dashboard_2.png"):
    ...
    ax2.set_ylim(0, 0.07)  # 設定 y 軸範圍（百分比）
    ax2.set_yticks([i / 100 for i in range(0, 8, 1)])
    ax2.set_yticklabels([f"{i}%" for i in range(0, 8, 1)])
    ...
    # 標題與佈局調整
    plt.title("診所點擊總數與轉換率", fontsize=16)
    fig.tight_layout()
